...
crimes.aisp %>% ggplot(aes(x = as.factor(Regiao))) + geom_boxplot(aes(y = roubo_transeunte))+ xlab('Batalhões') + ylab('Roubo à Transeunte') + facet_wrap(~mes.ano)

```

```{r calculando o r-quadrado para essa relação, echo = FALSE}
crimes.aisp %>% dplyr::group_by(mes.ano) %>% do(modelo = lm(roubo_transeunte ~ as.factor(Regiao), .)) %>% dplyr::mutate(r.ao.quadrado = summary(modelo)$r.squared) %>% dplyr::select(-modelo) %>% kable()
```
...
```{r calculando dispersao para as duas datas, echo = TRUE}
...
```{r calculando dispersao para análise de pearson em relação não linear, echo = TRUE}
...
```{r calculando dispersao para análise de separman em relação não linear, echo = TRUE}
